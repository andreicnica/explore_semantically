...
def make_gaussian(size, fwhm = 3, center=None, x=None, y=None):
...
class GaussianSegmentationMap:
    def __init__(self, size, no_classes):
        self.size = size
        self.no_classes = no_classes
        self.x = np.arange(0, size, 1, np.float32)
        self.y = np.arange(0, size, 1, np.float32)[:, np.newaxis]
    ...
    def __call__(self, image, boxes, labels):
        ...
        fwhm = np.max([cx, cy, size - cx, size - cy], axis=0)
        temp = (((width * height) / (size ** 2)) ** 0.5) * 2

        segmentation = np.zeros((no_classes, size, size), dtype=np.float32)
        ...
        for idx in range(boxes.shape[0]):
            ...
            gauss = make_gaussian(size, fwhm=fwhm[idx], center=(cx[idx], cy[idx]), x=x, y=y)
            s = softmax(gauss, temp=temp[idx])
            s *= 1. / s.max()
...
class Expand(object):
    def __init__(self, mean, max_expand: int = 4.):
        self.mean = mean
    ...
    def __call__(self, image, boxes, labels):
        ...
        ratio = random.uniform(1, max_expand)
...
class SSDAugmentation(object):
    ...
    def __call__(self, img, boxes, labels):
        img, boxes, labels = self.augment_base(img, boxes, labels)

        imgs = [x[0] for x in [scale(img) for scale in self.resize]]

        segmentation = [self.segmenter(imgs[0], boxes, labels)]
        if self.scale_segmentation:
            segmentation += [scale(segmentation[0]) for scale in self.resize[1:]]

        # TODO resize input images

        return imgs, boxes, labels, segmentation
